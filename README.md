...
> "ERD에 OpenAPI 명세까지 있으면 충분하지 않나?"  
> 충분하지 않다. **계약서와 실제 동작은 다른 것**이기 때문이다.

### ERD가 놓치는 것

ERD는 "데이터가 어떻게 저장되는가"만 표현한다.

```
ERD 가 보여주는 것:
  StateDB ──(1:N)── mappings
  StateDB ──(1:N)── corrections

ERD 가 못 보여주는 것:
  handlers.py::get() ──→ StateDB::get_all_mappings()   [calls]
  handlers.py::get() ──→ StateDB::_connect()            [중복 호출 × 5]
  classify_functions ──→ get()                          [fan-in = 5]
```

함수가 DB를 몇 번, 어떤 순서로 호출하는지는 ERD에 흔적조차 없다.

### ERD + API 명세를 더해도 놓치는 것

OpenAPI 명세가 더해지면 "인터페이스 계약"은 완성된다. 하지만 **계약서는 거짓말할 수 있다.**

```
OpenAPI 명세:  POST /api/confirm   ← 문서에 존재
실제 라우트:    route("/state")     ← 코드에 등록된 경로

stateStore.ts → /confirm [fetch] → ❌ 라우트 없음 → 런타임 404
```

명세와 구현이 드리프트된 순간, 버그는 **그래프에서만 보인다.**  
명세는 수동으로 작성·유지된다. 그래프는 코드에서 자동 추출된다.

### 세 가지 접근의 커버리지 비교

| 질문 | ERD | ERD + API 명세 | 의존성 그래프 |
| --- | --- | --- | --- |
| 데이터가 어떻게 저장되나? | ✅ | ✅ | ✅ |
| 어떤 API 엔드포인트가 있나? | ❌ | ✅ | ✅ |
| 명세와 구현이 일치하나? | ❌ | ❌ | ✅ (자동 검출) |
| 이 함수가 어디서 호출되나? (fan-in) | ❌ | ❌ | ✅ |
| 수정 시 어디까지 영향받나? | ❌ | ❌ | ✅ |
| 단일 장애점(SPOF)이 어디인가? | ❌ | ❌ | ✅ |
| 설계 결정의 WHY가 어디 있나? | ❌ | ❌ | ✅ (`rationale_for` 엣지) |
| 버전 충돌 가능성이 있나? | ❌ | ❌ | ✅ (manifest 파싱) |

### 핵심 차이 한 줄

| | 표현 대상 | 작성 주체 | 실제와 일치 보장 |
| --- | --- | --- | --- |
| ERD | 저장 구조 | 사람 (수동) | ❌ |
| ERD + API 명세 | 저장 + 인터페이스 | 사람 (수동) | ❌ |
| **의존성 그래프** | **동작 전체** | **코드 자동 추출** | **✅** |

> **계약서가 아니라 지문이다.**  
> 코드가 진실이면 그래프도 진실이다.

**실증 근거**: [Codebase-Memory, arXiv:2603.27277](https://arxiv.org/abs/2603.27277) — 31개 레포 측정, 파일 탐색 대비 **토큰 10× 감소**, **tool call 2.1× 감소**  
**이론적 근거**: [GraphRAG, arXiv:2404.16130](https://arxiv.org/abs/2404.16130) — Microsoft Research, 그래프 기반 쿼리의 이점 정량화
...
## 이력 기반 Hook 자동 학습 — 차별화 포인트

> 기존 린터(ESLint, Pylint)는 **범용 규칙**을 제공한다.  
> 이 시스템은 **이 팀이 실제로 반복한 실수**에서 규칙을 추출한다.

### 핵심 아이디어

```
세션 1: stateStore.ts URL 불일치 → 런타임 404 발생
         ↓ hook 발동 기록
세션 2: 동일 패턴 재발 → hook이 차단
         ↓ 발동 빈도 누적
세션 N: "이 팀은 URL 드리프트를 자주 만든다" → 규칙 강도 자동 상향
```

실수가 쌓일수록 가드레일이 정교해진다.

### 아키텍처

```
[hook 발동 로그]
  ↓ 패턴 추출 (어떤 규칙이, 어느 파일에서, 몇 번 발동됐는가)
[실수 빈도 맵]
  ↓ 임계값 도달 시 규칙 승격
[WARN → BLOCK 자동 승격]
  ↓ 그래프 쿼리로 유사 패턴 파일 탐색
[영향 범위 확장]  ← 의존성 그래프가 여기에 쓰인다
  ↓ 다음 세션부터 적용
[개인화된 가드레일]
  ↓ 반복 피드백
[재귀적 업데이트]
...
|---|---|---|---|
| 규칙 출처 | 커뮤니티 범용 | 팀이 수동 작성 | **이 팀의 실수 이력에서 자동 추출** |
| 업데이트 방식 | 버전 릴리스 | 사람이 편집 | **발동 빈도 기반 자동 재보정** |
| 컨텍스트 인식 | 없음 | 없음 | **의존성 그래프로 영향 범위 파악** |
| 강도 조절 | on/off | on/off | **WARN → BLOCK 점진적 승격** |
| 반복 실수 감지 | 없음 | 없음 | **세션 간 누적 패턴 추적** |

### 차별화가 되는 이유

일반 코딩 평가 플랫폼은 제출 결과만 본다.  
이 시스템은 **어떤 실수를 얼마나 반복하는가**를 본다.

```
평가자가 알고 싶은 것:
  "이 개발자가 구조적 실수를 반복하는 패턴이 있는가?"

기존 방법:   코드 리뷰 → 리뷰어의 기억에 의존
이 시스템:   hook 발동 이력 → 수치화된 패턴 → 자동 리포트
```

두 개발자가 같은 점수를 받았을 때, 한 명은 URL 불일치를 30번 반복했고  
다른 한 명은 처음부터 API 명세를 그래프와 대조했다면 — 그 차이가 보인다.

---

### 학술 근거

**① 그래프 기반 쿼리가 토큰을 줄이는 원리**

> [safishamsi/graphify](https://github.com/safishamsi/graphify) — Tree-Sitter AST → KG → BFS 서브그래프 반환.  
> [arXiv:2505.16901 — Code Graph Model](https://arxiv.org/pdf/2505.16901) — 코드 토큰을 그래프 노드로 압축, LLM 유효 컨텍스트 **512× 확장**.

실수 패턴 탐색 시 전체 코드를 읽지 않고 그래프 쿼리만으로 유사 파일을 찾는다.  
→ 패턴 추출 비용 최소화.

**② 독립 실증: 그래프 기반 탐색의 효율**

> [arXiv:2603.27277 — Codebase-Memory](https://arxiv.org/abs/2603.27277) — 31개 레포 실측.  
> 파일 탐색 대비 **토큰 10× 감소, tool call 2.1× 감소**, 정답률 83% (파일탐색 92% 대비 -9%p).

정답률 -9%p의 손실로 토큰 10×를 절감한다. 패턴 추출은 "정확한 완독"이 아니라  
"빠른 구조 파악"이 목적이므로 이 trade-off는 수용 가능하다.

**③ Hook 강도 조절이 필수인 이유**

> [arXiv:2603.18059 — Guardrails as Infrastructure](https://arxiv.org/html/2603.18059v1)

| Policy 강도 | 오남용 방지율 (VPR) | Task 성공률 |
|---|---|---|
| 허용적 (P1) | 0.000 | 0.356 |
| 엄격 (P4) | 0.681 | **0.067** |

→ 모든 규칙을 BLOCK으로 시작하면 task success가 18% 수준으로 붕괴한다.  
WARN에서 시작해 빈도 기반으로 BLOCK으로 승격하는 이유가 여기에 있다.
